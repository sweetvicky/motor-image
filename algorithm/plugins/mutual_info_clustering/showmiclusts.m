% showmiclusts() - Visualize getmiclusts() clusters. Opens several figures:
...
%              >>  clusters = getmiclusts(mutual_info);
%        Then
%              >>  showmiclusts(clusters)
%        else  >>  showmiclusts(clusters,compplotfunc)
% Input:
%
% clusters:    - cluster structure created by getmiclusts()
%
% Optional Input:
%
...
%  >>   showmiclusts(EEG.miclust,plotTopo);
%
% See also: getmiclusts(), eeg_miclust(), mi_pairs(), mdscale(), silhouette()
%
...
function showmiclusts(miclusters,plotComponentFunction)

if ~isstruct(miclusters)
   error('miclusters input must be a getmiclusts() output structure');
end

global EEG; % used to check if we are dealing with EEG components, primarily in component properties pop_prob()
...
%  Plot 3-D Clusters
%
figure('Name','Clusters in 3D','NumberTitle','off');
...
if size(t3d_forcluster,1)>1
  ...
  if isempty(p) || min(p)/max(p)>.05 % check if some are too close. If so, 
    ...
    miclusters.cluster{i}.t2d_for_axes = t2d_for_axes;
end;
...
for cluster_counter = 1:number_of_clusters
    ...
    for i=1:size(comp_cluster{cluster_counter},1)
        ...
        mapAxisSize = 0.09 * silhAxisSize*1.5;
      ...
      %             round((comp_cluster{cluster_counter}(i,2))*size(im,2)));
        ...
        if nargin>1
          if nargin(plotComponentFunction)==1
            ...
            if ~isempty(EEG)
                set(gca, 'userdata',['close(gcf);global EEG; pop_prop(EEG, 0,' num2str(comp_cluster{cluster_counter}(i,3)) ');']);
            end;
          else
